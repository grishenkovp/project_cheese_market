...
class PostgresDB():
    ...
    def read_table_pandas(self, script_sql: str) -> None:
        """Запрос данных из БД. Вывод в датафрейм Pandas"""
        point = self.create_point_connect()
        conn = psycopg2.connect(point)
        return pd.read_sql(script_sql, conn)

    def write_table_csv(self, name_dataset: str, name_db_table: str) -> None:
        ...
        df = df[df['date'] <= m.DATE_FINISH]
        df.to_sql(name_db_table, engine, if_exists='append', index=False)

    def write_table_dataframe(self, df, name_db_table: str) -> None:
    ...
    def create_file_csv(self, df, path_data_dataset: str, name_dataset: str) -> None:
